...
select Student.StudentName, Student.ClassId,ClassName
from Student
join Class on Student.ClassId = Class.ClassId;
-- 2. Lấy ra thông tin các học viên, và cho biết các học viên đang theo học lớp nào, lớp đó thuộc loại lớp nào.
select Student.StudentName, Student.ClassId, Class.ClassName,Class.status
from Student
join Class on Student.ClassId = Class.ClassId;
-- 4. Lấy thông tin của các học viên tên 'Tien' và 'Toan'.
select StudentName
from Student
where StudentName = 'Tien' and StudentName = 'Toan';
-- 5. Lấy ra số lượng học viên của từng lớp.
select count(StudentId), Class.ClassName
From Student
join Class on Student.ClassId = Class.ClassId
group by ClassName;
